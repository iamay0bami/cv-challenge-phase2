...
│   ├── outputs.tf            # Public IP and connection details
│   └── variables.tf           
├── ansible/                  # Configuration & Deployment
│   ├── roles/
│   │   ├── common/           # Docker installation & Version Locking
│   │   ├── app_stack/        # App deployment (Frontend, Backend, DB, Traefik)
│   │   └── monitoring_stack/ # Observability (Prometheus, Grafana, Loki)
│   ├── playbook.yml          # Main entry point for Ansible
│   └── inventory.ini         # Automatically generated by Terraform
└── README.md
```
...
- **The Fix:**  
  ...
  This ensures the environment remains stable and fully compatible with Traefik’s Docker provider.

---

### 2. Automated Inventory Management

Terraform manages the *“handshake”* between infrastructure and configuration by dynamically generating the `inventory.ini` file using a `local_file` resource.  
